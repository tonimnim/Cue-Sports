...
class _TournamentScreenState extends State<TournamentScreen>
  ...
  void _updateFeaturedTournament() {
    ...
    final upcomingTournaments =
        _tournaments.where((t) => _parseDate(t.date).isAfter(now)).toList();
          ...
          child: TabBarView(
            ...
            children: [
              _buildTournamentList(
                  upcoming: false,
                  past: false,
                  featuredTournament: _featuredTournament),
  ...
  bool _isUserAllowedToAccessTournament(Tournament tournament) {
    ...
    if (tournament.isPublic) {
      print(
          'DIRECT-SCREEN: Tournament ${tournament.name} is public - ACCESS GRANTED');
      return true;
    }

    print(
        'DIRECT-SCREEN: Tournament ${tournament.name} is NOT public - checking communities');
    ...
    if (tournament.communities.isEmpty) {
      print(
          'DIRECT-SCREEN: Tournament ${tournament.name} has no communities but is not public - ACCESS DENIED');
    ...
    if (tournament.communities.contains(_userCommunity)) {
      print(
          'DIRECT-SCREEN: Tournament includes user\'s community $_userCommunity - ACCESS GRANTED');
      return true; // Tournament is available for the user's community
    }

    print(
        'DIRECT-SCREEN: User\'s community $_userCommunity is not included in tournament communities - ACCESS DENIED');
  ...
  List<Tournament> _getFilteredTournaments({
    ...
    print(
        'DIRECT-SCREEN: Getting filtered tournaments: search="$searchQuery", upcoming=$upcoming, past=$past');
    print(
        'DIRECT-SCREEN: Total tournaments before filtering: ${_tournaments.length}');
    ...
    final result = _tournaments.where((tournament) {
      ...
      final isUpcomingOrOngoing =
          tournamentStartDate.isAfter(now) || tournamentEndDate.isAfter(now);

      print(
          'DIRECT-SCREEN: Tournament ${tournament.name} - Start: ${tournament.startDate}, End: ${tournament.endDate}, isPast: $isPast, isUpcomingOrOngoing: $isUpcomingOrOngoing');
    ...
    if (result.isNotEmpty) {
      print(
          'DIRECT-SCREEN: Tournaments after filtering and sorting: ${result.length}');
      print('DIRECT-SCREEN: Top 3 tournaments by update date:');
      for (int i = 0; i < (result.length > 3 ? 3 : result.length); i++) {
        print(
            'DIRECT-SCREEN:   ${i + 1}. ${result[i].name} - Updated: ${result[i].updatedAt}');
      }
    }
    return result;
  }
  ...
  Widget _buildTournamentList(
    ...
    print(
        'DIRECT-SCREEN: Building tournament list: upcoming=$upcoming, past=$past, search="$_searchQuery"');
          ...
          if (featuredTournament != null && !upcoming && !past)
            Padding(
              ...
              child: TournamentCard(
                ...
                dateRange:
                    featuredTournament.dateRange, // Use the dateRange property
                players: featuredTournament.players,
                price: featuredTournament.price,
                isFeatured: true,
                isRegistered:
                    featuredTournament.registeredUsers.contains(_userId),
                isPast: _parseDate(featuredTournament.endDate)
                    .isBefore(DateTime.now()), // Past if end date is before now
                onRegisterPressed: () async {
                  ...
                  if (isRegistered) {
                    ...
                    if (result == true) {
                      final success =
                          await _tournamentService.registerUserForTournament(
